import streamlit as st
import pandas as pd
import random

st.set_page_config(page_title="🔥 요즘 뭐가 유행할까?", layout="wide")

# ------------------ 샘플 데이터 ------------------
items = ["제로콜라", "크록스", "슬리퍼 패션", "오버핏 셔츠", 
         "프롬프트 엔지니어", "몬스터 에너지", "진또배기 밈", 
         "일본 라멘", "헬스 보충제", "하이볼"]

# 년도별 검색량 데이터 (2019~2025)
years = list(range(2019, 2026))
trend_data = {"년도": years}
for item in items:
    trend_data[item] = [random.randint(30, 100) for _ in years]

df_year = pd.DataFrame(trend_data)

# 현재 시점 데이터
df_now = pd.DataFrame({
    "항목": items,
    "검색량": [random.randint(50, 100) for _ in items],
    "판매량": [random.randint(50, 100) for _ in items],
    "인기도": [random.randint(50, 100) for _ in items],
})
df_now["종합점수"] = df_now[["검색량", "판매량", "인기도"]].mean(axis=1)

# ------------------ 제목 ------------------
st.title("✨ 요즘 뜨는 트렌드 분석기")
st.write("검색량, 판매량, 인기도 데이터를 기반으로 **시간에 따른 유행 변화**를 확인해보세요!")

# ------------------ 인기 TOP 3 ------------------
st.subheader("🔥 지금 가장 핫한 TOP 3")
top3 = df_now.sort_values("종합점수", ascending=False).head(3)

for i, row in top3.iterrows():
    st.markdown(f"### 🏆 {row['항목']}  —  점수: {row['종합점수']:.1f}")

# ------------------ 시간별 변화 ------------------
st.subheader("⏳ 시간에 따른 트렌드 변화")
selected_items = st.multiselect("시간 변화가 궁금한 항목을 선택하세요:", items, default=top3["항목"].tolist())

if selected_items:
    st.line_chart(df_year.set_index("년도")[selected_items])

# ------------------ 상세 추천 ------------------
st.subheader("💡 트렌드 분석 및 추천")
choice = st.selectbox("관심 있는 분야를 골라보세요:", ["음식", "패션", "밈", "인물", "기타"])

if choice == "음식":
    st.success("🍜 요즘 라멘, 하이볼 같은 **일본식 음식과 주류**가 크게 뜨고 있어요!")
elif choice == "패션":
    st.success("👟 슬리퍼 패션, 오버핏 셔츠가 여름철 핫 아이템이에요!")
elif choice == "밈":
    st.success("😂 '진또배기' 같은 짧고 중독성 강한 밈이 대세!")
elif choice == "인물":
    st.success("🤖 '프롬프트 엔지니어' 같은 AI 관련 직업이 뜨는 중!")
else:
    st.info("📈 다양한 트렌드가 동시에 확산 중이니, 복합적인 관심사가 유리해요!")

# ------------------ 데이터 테이블 ------------------
st.subheader("📋 원본 데이터 (현재 시점)")
st.dataframe(df_now)

st.subheader("📋 시간별 검색량 데이터 (년도별)")
st.dataframe(df_year)


# ------------------ 데이터 테이블 ------------------
st.subheader("📋 원본 데이터")
st.dataframe(df)

